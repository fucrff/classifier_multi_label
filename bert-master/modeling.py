...
class BertModel(object):
  ...
  def __init__(self,
    ...
    if input_mask is None: #如果没设置mask，自动添加mask，自然就都是1的
      input_mask = tf.ones(shape=[batch_size, seq_length], dtype=tf.int32)

    if token_type_ids is None:   # 如果没说是两句话默认为一句话
      token_type_ids = tf.zeros(shape=[batch_size, seq_length], dtype=tf.int32)

    with tf.compat.v1.variable_scope(scope, default_name="bert"):
      with tf.compat.v1.variable_scope("embeddings"):      # 构建embedding层：词嵌入，对每一个词，type_id和位置信息做embedding
        # Perform embedding lookup on the word ids.
        (self.embedding_output, self.embedding_table) = embedding_lookup(
            input_ids=input_ids,
            vocab_size=config.vocab_size,     # 语料库大表
            embedding_size=config.hidden_size,    # 想要把词映射成多少维，官方文档给出为768维（判断两个句子是不是一个意思的任务）
...
def embedding_lookup(input_ids,
  ...
  embedding_table = tf.compat.v1.get_variable( #词映射矩阵，30522, 768
...
def embedding_postprocessor(input_tensor,
  ...
  if use_token_type:
    ...
    token_type_table = tf.compat.v1.get_variable(#(2, 768)
...
def attention_layer(from_tensor,
  ...
  # `query_layer` = [B*F, N*H]    查询矩阵，由from-tensor构建的[1024*768]
  ...
  # `value_layer` = [B*T, N*H] 实际得到特征应该是什么
  ...
  if attention_mask is not None:   # 做soft-max的时候需要关注一下 attition_mask的问题
...
def transformer_model(input_tensor,
  ...
  for layer_idx in range(num_hidden_layers):
    with tf.compat.v1.variable_scope("layer_%d" % layer_idx):
      layer_input = prev_output    # 当前输入等于上一层的输出：第一层self-attition的输入是embedding那一层的输出结果

      with tf.compat.v1.variable_scope("attention"):
        attention_heads = []
        with tf.compat.v1.variable_scope("self"):
          attention_head = attention_layer(     # self-attition
              from_tensor=layer_input,          # 第一个句子可以和第二个句子做attition，但是self-attition是和自己这一句话中的每一个做attition
              to_tensor=layer_input,            # 所以from_tensor和to_tensor是一致的（self-attition）
              attention_mask=attention_mask,
              ...
              from_seq_length=seq_length,   # 最大长度128
              to_seq_length=seq_length)     # 最大长度128
        ...
        # with `layer_input`.
        with tf.compat.v1.variable_scope("output"): #1024, 768 残差连接
      ...
      with tf.compat.v1.variable_scope("intermediate"): #全连接层 (1024, 3072)
      ...
      with tf.compat.v1.variable_scope("output"): #再变回一致的维度
